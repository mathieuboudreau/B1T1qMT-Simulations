function [] = b1t1qmtSimu_Stage2_singleline(outputDir,mt_measurement,vol_dims,vol_resolution,protocol, sled_trs, sled_excite_flips)
...
mask=zeros(vol_dims(2), vol_dims(1));
mask(1,:)=1;
MASK = minc;
...
%
%Set No-MT volumes
qmt_m0_vol=ones(vol_dims(2), vol_dims(1));
...
t1_range=linspace(0.9,0.9,vol_dims(1));
...
vfa_vol{1}=repmat(VFASignal(:,1),1,vol_dims(2))';
VFA1 = minc;
...
vfa_vol{2}=repmat(VFASignal(:,2),1,vol_dims(2))';
VFA2 = minc;
